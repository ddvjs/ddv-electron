...
Object.assign(ddvelectron, {
  ...
  register () {
    ...
    protocol.registerStreamProtocol('ddvelectron', this.handler.bind(this), this.completion.bind(this))
  },
  // 完成
  completion (error) {
    if (error) {
  ...
  handler (request, callback) {
    ...
    if (url.substr(0, '/_nuxt/'.length) === '/_nuxt/') {
      ...
      callback(fs.createReadStream(path.resolve(argv['root-dir'], '.nuxt/dist/', path1)))
      return
    }
